...
class TextFormattingHelper {
    ...
    static func applyBold(to textView: UITextView, range: NSRange) {
    ...
    static func applyItalic(to textView: UITextView, range: NSRange) {
        let attributedString = NSMutableAttributedString(attributedString: textView.attributedText)
    ...
    static func applyUnderline(to textView: UITextView, range: NSRange) {
        let attributedString = NSMutableAttributedString(attributedString: textView.attributedText)
    ...
    static func applyStrikethrough(to textView: UITextView, range: NSRange) {
        let attributedString = NSMutableAttributedString(attributedString: textView.attributedText)
    ...
    static func changeTextColor(to textView: UITextView, range: NSRange, color: UIColor) {
        let attributedString = NSMutableAttributedString(attributedString: textView.attributedText)
    ...
    /// Applies dash list
    static func applyDashList(to textView: UITextView) {
        let text = textView.text ?? ""
        let newText = text.components(separatedBy: "\n").map { "- " + $0 }.joined(separator: "\n")
        textView.text = newText
    }
    ...
    static func applyNumberedList(to textView: UITextView) {
        let text = textView.text ?? ""
    ...
    /// Aligns text to left
    static func alignLeft(to textView: UITextView) {
    ...
    static func alignRight(to textView: UITextView) {
        let paragraphStyle = NSMutableParagraphStyle()
    ...
    static func applyMonospacedFont(to textView: UITextView, range: NSRange) {
    ...
    static func applyHeadingStyle(to textView: UITextView, range: NSRange) {
        guard range.length > 0 else { return } // Ensure text is selected
    ...
    /// Applies title style
    static func applyTitleStyle(to textView: UITextView, range: NSRange) {
        guard range.length > 0 else { return } // Ensure text is selected
    ...
    static func applySubHeadingStyle(to textView: UITextView, range: NSRange) {
        guard range.length > 0 else { return } // Ensure text is selected
    ...
    /// Applies Body style
    static func applyBodyStyle(to textView: UITextView, range: NSRange) {
        guard range.length > 0 else { return } // Ensure text is selected
